...
BATTERIES = {
    Battery.agm:        BatteryDetail("AGM", [10.5, 11.51, 11.66, 11.81, 11.95, 12.05, 12.15, 12.3, 12.5, 12.75, 12.8, 12.85], 20, 30, 13.6, 14.3),
    Battery.deepcycle:  BatteryDetail("Deep-cycle", [10.5, 11.51, 11.66, 11.81, 11.95, 12.05, 12.15, 12.3, 12.5, 12.75, 12.8], 20, 50, 13.6, 14.4),
    Battery.leadacid:   BatteryDetail("Lead-acid", [10.5, 11.31, 11.58, 11.75, 11.9, 12.06, 12.2, 12.32, 12.42, 12.5, 12.7], 50, 60, 13.7, 14.5),
    Battery.lifepo4:    BatteryDetail("LiFePO4", [10.0, 12.0, 12.5, 12.8, 12.9, 13.0, 13.1, 13.2, 13.3, 13.4, 13.6], 5, 20, 13.5, 14.4),
    Battery.lithiumion: BatteryDetail("Lithium-ion", [10.0, 12.0, 12.8, 12.9, 13.0, 13.05, 13.1, 13.2, 13.3, 13.4, 13.6], 20, 30, 13.5, 14.25)
}
...
class BMxBluetoothDeviceData(BluetoothData):
    ...
    async def _get_payload(self, client: BleakClientWithServiceCache) -> None:
        ...
        if self._gattdata is not None:
            ...
            if battery_chemistry != "Automatic (from BM2)":
            ...
            if status >= 4: #float or charging, ie attached to a powered-on charger
                self._charging = True
            else:
    ...
    def _adjust_percentage(self, raw_percentage: int, battery_detail, voltage: float) -> int:
        ...
        _LOGGER.debug ("Adjusting percentage based on battery chemistry of %s: current voltage = %s, raw percentage = %s", battery_detail.battery_chemistry, str(voltage), str(raw_percentage))
    ...
    def _adjust_status(self, raw_status: int, battery_detail, voltage: float) -> int:
        ...
        _LOGGER.debug ("Adjusting state based on battery chemistry of %s: critical voltage = %s, low voltage = %s, float voltage = %s, charging voltage = %s, raw voltage = %s", battery_detail.battery_chemistry, str(battery_detail.critical_voltage), str(battery_detail.low_voltage), str(battery_detail.floating_voltage), str(battery_detail.charging_voltage), str(voltage))
        if voltage >= battery_detail.charging_voltage:
            return 8    # Charging
        elif voltage >= battery_detail.floating_voltage:
            return 4    # Floating
        elif volrage <= battery_detail.critical_voltage:
            return 0    # Critical
